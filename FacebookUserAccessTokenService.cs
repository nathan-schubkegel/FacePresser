  ...
  public static async Task<string> GetUserAccessToken()
  {
    if (File.Exists(Constants.FacebookUserAccessTokenFileName))
    {
      ...
      try
      {
        ...
        var json = JsonConvert.DeserializeObject<CachedToken>(
          text,
      ...
      {
        Console.WriteLine(
          "Unexpected " + ex.GetType().Name + " while inspecting user access token from file... abandoning it."
        );
      }
        ...
        try
        {
          ...
          else
          {
            ...
            try
            {
              ...
              {
                Console.WriteLine(
                  $"facebook returned error status code {(int)response.StatusCode} ({response.StatusCode})"
                );
                userAccessToken.SetException(new Exception("facebook code decryption failed"));
              }
            }
